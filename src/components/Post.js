...
export default function Post({ date, excerpt, thumbnail, title, category, slug }) {
  ...
  return (
    <article className="px-4 py-2  cursor-pointer overflow-hidden h-full   bg-cyan-950   hover:bg-cyan-500 rounded-lg shadow-lg  hover:shadow-cyan-950 shadow-cyan-700 transition duration-300 hover:shadow-xl">
      <Link href={slug} className="">
        ...
        {formattedDate && (
          <div className="flex space-x-2 mb-2 sm:mb-4 lg:mb-6">
            ...
            {/*   {category && ( */}
            <span className="text-xs text-cyan-50 bg-cyan-600  rounded-full">{category} </span>
            {/*    )} */}
          </div>
        )}
        <div className="flex">
          ...
          <div
            className={`font-roboto leading-relaxed text-med max-w-screen-md mx-auto ${thumbnail ? 'w-2/3' : 'w-full'
